...
public class CreateOrderTest {
    ...
    @DisplayName("Успешное создание заказа с авторизацией и с валидными ингредиентами")
    ...
    @DisplayName("Успешное создание заказа без авторизации и с валидными ингредиентами")
    @Description("Статус кода: 200, В теле ответа есть success (true), номер заказа и имя")
    ...
    @DisplayName("Не создается заказ при передаче невалидного хеша ингредиента")
    ...
    public void orderWithInvalidIngredientsDontBeCreated() {
        ...
        accessToken = userClient.create(createUserRequest)
                .extract().jsonPath().get("accessToken");
        //попытка создания заказа c невалидным ингредиентом
